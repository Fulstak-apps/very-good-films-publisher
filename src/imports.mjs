...
export async function importPrepared(memory,directory='inbox'){
 ...
 for(const name of (await fs.readdir(directory).catch(e=>{if(e.code==='ENOENT')return [];throw e;})).filter(n=>n.endsWith('.json'))){
  ...
  const x={kind:raw.kind,film:raw.film,scene:raw.scene,source_post_url:raw.source_post_url,source_caption:raw.source_caption,source_details:await enrichSourceMetadata(raw.source_caption,raw.source_details),video_url:raw.video_url,asset_sha256:raw.asset_sha256,qa:raw.qa,status:'ready',discovered_at:new Date().toISOString()};
