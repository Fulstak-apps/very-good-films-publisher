...
export async function discover(memory,sources){
 ...
 if(feedOnly){for(const item of memory.items){if(!approvedSource(item.source_post_url)&&!item.instagram_published_at&&['discovered','ready','needs_review'].includes(item.status)){item.status='retired';item.retired_reason='Catalog disabled; waiting for approved source feed';}}}
