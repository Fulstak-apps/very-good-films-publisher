...
export function enforceSourcePolicy(items) {
 ...
 for(const x of items){
  ...
  if(x.program==='public_domain_classics'&&x.rights?.status==='public_domain'&&x.qa?.black_and_white===true&&x.qa?.frame_preserved===true&&x.qa?.branding==='very-good-films-only-v1')continue;
