...
async function commit(){
 ...
 // `state/memory.json` is generated by the publisher and can be dirty locally
 // after a remote Actions run. It is never collector-owned, so restore that
 // generated file before syncing instead of letting it block the refill loop.
 await runCommand('git',['restore','--source=HEAD','--','state/memory.json'],{env:commandEnv});
